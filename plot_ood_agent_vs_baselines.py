"""Plot OOD g-mean² for agent vs best monitor vs best non-monitor baseline."""
...
def best_ood_gmean2(methods: dict[str, dict[str, tuple[float, float]]], *, llm_only: bool) -> tuple[str, float]:
...
def score_agent_run(run: Path) -> tuple[float, int, int]:
    ...
    return tpr * tnr, int(len(yt)), sum(1 for _ in (REPO / "data" / run.parent.name / "test").glob("*.json"))
...
def draw_rounded_bar(ax, cx, bottom, height, color, width, r_x, r_y, *, zorder=3):
    ...
    ax.add_patch(mpatches.PathPatch(mpath.Path(verts, codes), facecolor=color, edgecolor="none", zorder=zorder))
...
def main():
    ...
    rows = []
    print(f"{'task':30s} {'agent':>6s}  {'best monitor (OOD)':28s}  {'best non-monitor (OOD)':28s}")
    for csv_task, run_task, label in TASKS:
        ...
        rows.append({
            ...
            "nonmonitor": nonmonitor_val,
        })
        print(
            f"{csv_task:30s} {agent_val:6.3f}  "
    ...
    ymax = max(max(r["agent"], r["monitor"], r["nonmonitor"]) for r in rows)
    ...
    for i, row in enumerate(rows):
        specs = [
            (x[i] + offsets[0], row["monitor"], ORANGE_DARK),
            (x[i] + offsets[1], row["agent"], GREEN_DARK),
            (x[i] + offsets[2], row["nonmonitor"], BLUE_DARK),
        ]
    ...
    handles = [
        ...
        mpatches.Patch(facecolor=GREEN_DARK, edgecolor="none", label="Agent (OOD)"),
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f"{r['label']}\nn={r['agent_n']}/{r['agent_total']}" for r in rows], fontsize=9.5, color=TEXT)
    ...
    ax.set_title("OOD agent vs. best monitor vs. best non-monitor", fontsize=13, color=TEXT, pad=10)
    ...
    fig.tight_layout()
    out = PLOTS_DIR / "ood_agent_vs_monitor_nonmonitor.png"
